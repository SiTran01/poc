...
{
    I2C_Type_Def *pI2Cx = pI2CHandle->pI2Cx;
    volatile uint32_t timeout_cnt = 0; // Bi?n d?m th?i gian ch?

    // 1. T?o di?u ki?n START
    pI2Cx->CR1 |= (1 << 8); 
    
    // 2. Ch? Start bit (SB=1) CÓ TIMEOUT
    timeout_cnt = 0;
    while( ! (pI2Cx->SR1 & (1 << 0)) )
    {
        timeout_cnt++;
        if(timeout_cnt > I2C_TIMEOUT) return; // <-- Quá lâu thì b?, thoát hàm ngay
    }
    ...
    // 4. Ch? d?a ch? g?i xong (ADDR=1) CÓ TIMEOUT
    // Ðây là ch? hay treo nh?t n?u rút dây OLED
    timeout_cnt = 0;
    while( ! (pI2Cx->SR1 & (1 << 1)) )
    {
        timeout_cnt++;
        // Ki?m tra xem có b? màn hình t? ch?i (NACK) không?
        if(pI2Cx->SR1 & (1 << 10)) { // C? AF (Acknowledge Failure)
            pI2Cx->SR1 &= ~(1 << 10); // Xóa l?i
            pI2Cx->CR1 |= (1 << 9);   // G?i STOP
            return; // Thoát ngay
        }
        if(timeout_cnt > I2C_TIMEOUT) return; // <-- H?t gi?, thoát
    }

    // 5. Xóa c? ADDR (Th? t?c chu?n: Ð?c SR1 r?i SR2)
    uint32_t dummyRead = pI2Cx->SR1;
    dummyRead = pI2Cx->SR2;
    (void)dummyRead; 
    ...
    while(Len > 0)
    {
        // Ch? d?m truy?n tr?ng (TXE=1) CÓ TIMEOUT
        timeout_cnt = 0;
        while( ! (pI2Cx->SR1 & (1 << 7)) )
        {
            timeout_cnt++;
            if(timeout_cnt > I2C_TIMEOUT) return;
        }
    ...
    // 7. Ch? TXE=1 và BTF=1 (Byte Transfer Finished) d? ch?c ch?n gói tin di h?t
    timeout_cnt = 0;
    while( ! (pI2Cx->SR1 & (1 << 7)) ) {
        timeout_cnt++; if(timeout_cnt > I2C_TIMEOUT) return;
    }
    timeout_cnt = 0;
    while( ! (pI2Cx->SR1 & (1 << 2)) ) {
        timeout_cnt++; if(timeout_cnt > I2C_TIMEOUT) return;
    ...
    // 8. T?o di?u ki?n STOP (K?t thúc truy?n)
    pI2Cx->CR1 |= (1 << 9); 
}
